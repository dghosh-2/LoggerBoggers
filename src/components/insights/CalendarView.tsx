...
export function CalendarView() {
    ...
    // Calculate Actual (Projection logic removed for now as import was missing)
    ...
    // Get intensity color based on amount (Light/Dark mode compatible)
    const getIntensity = (amount: number) => {
        if (amount === 0) return 'bg-card border-border';
        if (amount < 100) return 'bg-emerald-500/10 border-emerald-500/20';
        if (amount < 500) return 'bg-emerald-500/20 border-emerald-500/30';
        if (amount < 1500) return 'bg-emerald-500/40 border-emerald-500/50';
        return 'bg-emerald-500/60 border-emerald-500 shadow-md';
    };
    ...
    return (
        <div className="min-h-full w-full pt-16 p-8 pb-24 flex flex-col items-center">
            ...
            <div className="flex items-center justify-between w-full max-w-4xl mb-6">
                <div>
                    ...
                    <div className="flex items-center space-x-2">
                        <span className="text-sm font-mono font-bold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-600 dark:text-emerald-400">
                            TOTAL SPENT
                        </span>
                ...
                <div className="flex space-x-2">
                    <button onClick={handlePrevMonth} className="p-2 rounded-full border border-border hover:bg-secondary text-foreground-muted hover:text-foreground transition-colors">
                        <ChevronLeft className="w-4 h-4" />
                    </button>
                    <button onClick={handleNextMonth} className="p-2 rounded-full border border-border hover:bg-secondary text-foreground-muted hover:text-foreground transition-colors">
            ...
            <div className="w-full max-w-4xl grid grid-cols-7 gap-3 mb-8">
                {DAYS.map(day => (
                    <div key={day} className="text-center text-xs font-semibold text-foreground-muted uppercase tracking-widest py-2">
                        {day}
                    </div>
                ))}
                            ...
                            <>
                                <div className="flex justify-between items-start">
                                    <span className={cn(
                                        ...
                                        date.amount && date.amount > 1500 ? "text-foreground" : "text-foreground-muted"
                                    )}>
                                        {date.day}
                                    </span>
                                    {date.sub && (
                                        <div className="flex bg-blue-500/20 text-blue-600 dark:text-blue-300 text-[9px] px-1.5 py-0.5 rounded-full border border-blue-500/30 items-center">
                                ...
                                {date.amount !== undefined && date.amount > 0 && (
                                    <div className="mt-auto">
                                        <span className={cn(
                                            ...
                                            date.amount > 1500 ? "text-foreground" : "text-foreground-muted"
                                        )}>
            ...
            {selectedDayData && (
                <div className="w-full max-w-4xl bg-card border border-border rounded-2xl p-6 animate-in slide-in-from-top-4 duration-300 shadow-xl">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-foreground">
                            {selectedMonth} {selectedDayData.day}
                        </h3>
                        <button onClick={() => setSelectedDay(null)} className="p-1 hover:bg-secondary rounded-full text-foreground-muted">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                    ...
                    {selectedDayData.transactions && selectedDayData.transactions.length > 0 ? (
                        <div className="space-y-2">
                            {selectedDayData.transactions.map((t, i) => (
                                <div key={i} className="flex justify-between items-center p-3 rounded-lg bg-secondary/50 border border-border hover:border-strong transition-colors">
                                    <div className="flex items-center space-x-3">
                                        <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-xs font-bold text-foreground-muted">
