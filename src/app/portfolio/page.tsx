        ...
        <GlassCard>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              {totalStockValue > 0 && (
                <span className="text-xs text-foreground-muted">
                  (${totalStockValue.toLocaleString()} total)
                </span>
              )}
            </div>
            <p className="text-[10px] text-foreground-muted">
              Portfolio tab only · Not included in net worth
            </p>
          </div>

          {/* Add Stock Form */}
          <div className="p-4 rounded-lg bg-secondary/50 mb-4">
            <p className="text-xs font-medium mb-3">Add Stock to Portfolio</p>
              ...
              <div className="flex-1 relative">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2.5 rounded-lg bg-background border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
                    ...
                    {searchResults.map((result) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-[10px] text-foreground-muted truncate">{result.name}</p>
                        </div>
                        <span className="text-[10px] text-foreground-muted">{result.exchange}</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {/* Selected Stock Info */}
                {selectedStock && (
                  <div className="mt-2 p-2 rounded-md bg-primary/10 flex items-center justify-between">
                    <div>
                      <p className="text-xs font-medium">{selectedStock.symbol} - {selectedStock.name}</p>
                      <p className="text-[10px] text-foreground-muted">
                        Current price: ${selectedStock.price?.toFixed(2) || 'Loading...'}
                      </p>
                    </div>
                    <button
                      onClick={() => {
            ...
            {/* Value Preview */}
            {selectedStock && stockShares && parseFloat(stockShares) > 0 && (
              <div className="mt-3 p-2 rounded-md bg-success/10 text-center">
          ...
          {manualStockHoldings.length === 0 ? (
            <div className="text-center py-8">
              <LineChart className="w-10 h-10 text-foreground-muted mx-auto mb-2" />
              <p className="text-sm font-medium mb-1">No stocks added yet</p>
              <p className="text-xs text-foreground-muted">
                Add your stock holdings above to track their value
              </p>
            </div>
          ) : (
            <div className="space-y-2">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className={`text-[10px] tabular-nums ${holding.changePercent >= 0 ? 'text-success' : 'text-destructive'}`}>
